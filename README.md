...
```mermaid
flowchart TB
    User([User]) --> R53["Route 53<br/>api-origin domain"]
    R53 --> ALB["Public ALB<br/>/api/* routing"]
    ...
    subgraph AWS["AWS ap-northeast-2"]
        subgraph VPC["VPC 10.0.0.0/22"]
            ...
            subgraph App["Private App Subnets - 2 AZ"]
                API["ECS Fargate<br/>API Service"]
                FreeWorker["ECS Fargate<br/>Free Worker"]
                PaidWorker["ECS Fargate<br/>Paid Worker"]
                Jenkins["Jenkins EC2<br/>SSM only"]
                DBAdmin["DB Admin EC2<br/>SSM only"]
            end
            ...
            VPCE["VPC Endpoints<br/>S3 · ECR · Logs · Secrets<br/>SSM · STS"]
        end

        FreeQ[[SQS Free Queue]]
        PaidQ[[SQS Paid Queue]]
        DLQ[[Free / Paid DLQ]]
        Audio[(S3 Audio / Result)]
        Model[(S3 AI Model)]
        ECR[(Amazon ECR)]
    ...
    FreeQ --> FreeWorker
    PaidQ --> PaidWorker
    FreeQ -. failed 3 times .-> DLQ
    PaidQ -. failed 3 times .-> DLQ
    FreeWorker --> Audio
    PaidWorker --> Audio
    FreeWorker --> Model
    PaidWorker --> Model
    ...
    Scale --> PaidWorker
    App --> VPCE
    App -. fallback internet egress .-> NAT
```
